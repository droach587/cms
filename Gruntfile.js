...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    uglify: {
      build: {
        ...
        options: {
          ...
          sourceMap: true,
          sourceMapName: '<%= config.build %>/assets/js/build/app.min.js.map'
        }
      }
    },
    /**
    ...
    modernizr: {
      dist: {
        ...
        "dest": "<%= config.build %>/assets/js/vendor-non-build/modernizr-custom.js",
        "extra": {
          "shiv": true,
          "printshiv": false,
          "load": true,
    ...
    autoprefixer: {
      dist: {
        files: {
          '<%= config.build %>/assets/css/styles.css': '<%= config.build %>/assets/css/styles.css'
        }
      }
    },
  ...
  //Define Grunt Tasks
  grunt.registerTask('default', ['notify:test', 'clean', 'compass', 'newer:assemble', 'imagemin', 'copy', 'bower_concat:app', 'concat', 'uglify', 'modernizr', 'string-replace', 'htmlmin', 'autoprefixer', 'notify:testOk']);
};
